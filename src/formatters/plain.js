...
const typeActions = {
  ...
  changed: (key, oldValue, newValue, acc) => `Property '${acc}${key}' was updated. From '${renderObjectValue(
    oldValue,
  )}' to '${renderObjectValue(newValue)}'`,
  deleted: (key, oldValue, newValue, acc) => `Property '${acc}${key}' was removed`,
  added: (key, oldValue, newValue, acc) => `Property '${acc}${key}' was added with value: '${renderObjectValue(newValue)}'`,
};
const render = (ast) => {
